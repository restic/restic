...
func NewReader(ctx context.Context, bucket, name string) (io.ReadCloser, error) {
	return NewLimitedReader(ctx, bucket, name, 0, -1)
}

// NewLimitedReader creates a new io.ReadCloser to read only the contents of
// the object beginning at offset.  If length is -1, it reads to the end of the
// object.
func NewLimitedReader(ctx context.Context, bucket, name string, offset, length int64) (io.ReadCloser, error) {
	...
	if res.StatusCode < 200 || res.StatusCode > 299 {
		res.Body.Close()
		return nil, fmt.Errorf("storage: can't read object %v/%v, status code: %v", bucket, name, res.Status)
	}
