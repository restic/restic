...
// [cc](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#cc) query parameter.
// To determine the market to return results for, Bing uses the first supported language it finds from the list and
// combines it with the cc parameter value. If the list does not include a supported language, Bing finds the
// closest language and market that supports the request or it uses an aggregated or default market for the
// results. To determine the market that Bing used, see the BingAPIs-Market header. Use this header and the cc
...
// interface strings in the JSON response objects. Any links to Bing.com properties in the response objects apply
// the specified language. userAgent is the user agent originating the request. Bing uses the user agent to provide
// mobile users with an optimized experience. Although optional, you are encouraged to always specify this header.
// The user-agent should be the same string that any commonly used browser sends. For information about user
// agents, see [RFC 2616](http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html). The following are examples of
// user-agent strings. Windows Phone: Mozilla/5.0 (compatible; MSIE 10.0; Windows Phone 8.0; Trident/6.0;
...
// code of the country where the results come from. This API supports only the United States market. If you specify
// this query parameter, it must be set to us. If you set this parameter, you must also specify the Accept-Language
...
// not specify both. category is the category of articles to return. For example, Sports articles or Entertainment
// articles. For a list of possible categories, see [News Categories by
...
// both: Headline articles typically published in the last 24 hours from any category and articles from each parent
// category (up to four articles). If the article is a headline, the article's headline field is set to true. By
// default, the response includes up to 12 headline articles. To specify the number of headline articles to return,
// set the
// [headlineCount](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#headlineCount)
// query parameter. count is the number of news articles to return in the response. The actual number delivered may
// be less than requested. The default is 10 and the maximum value is 100. The actual number delivered may be less
// than requested.You may use this parameter along with the offset parameter to page results. For example, if your
// user interface displays 20 articles per page, set count to 20 and offset to 0 to get the first page of results.
// For each subsequent page, increment offset by 20 (for example, 0, 20, 40). It is possible for multiple pages to
...
// parameter, Bing ignores this parameter. headlineCount is the number of headline articles to return in the
// response. The default is 12. Specify this parameter only if you do not specify the
// [category](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#category)
// parameter. market is the market where the results come from. Typically, mkt is the country where the user is
...
// [cc](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#cc) query parameter
// are mutually exclusive—do not specify both. offset is the zero-based offset that indicates the number of news to
// skip before returning news. The default is 0. The offset should be less than
...
// page, increment offset by 20 (for example, 0, 20, 40). It is possible for multiple pages to include some overlap
// in results. If you do not specify the
// [category](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#category)
// parameter, Bing ignores this parameter. originalImage is a Boolean value that determines whether the image's
// contentUrl contains a URL that points to a thumbnail of the original article's image or the image itself. If the
// article includes an image, and this parameter is set to true, the image's contentUrl property contains a URL
// that you may use to download the original image from the publisher's website. Otherwise, if this parameter is
// false, the image's contentUrl and thumbnailUrl URLs both point to the same thumbnail image. Use this parameter
// only with the News Search API or News Category API. Trending Topics API ignore this parameter. safeSearch is
// filter news for adult content. The following are the possible filter values. Off: Return news articles with
// adult text, images, or videos. Moderate: Return news articles with adult text but not adult images or videos.
// Strict: Do not return news articles with adult text, images, or videos. If the request comes from a market that
...
// regardless of what the safeSearch query parameter is set to. Use site: only if you are aware of the content on
// the site and your scenario supports the possibility of adult content. setLang is the language to use for user
// interface strings. Specify the language using the ISO 639-1 2-letter language code. For example, the language
// code for English is EN. The default is EN (English). Although optional, you should always specify the language.
...
// Bing.com properties in the response objects apply the specified language. textDecorations is a Boolean value
// that determines whether display strings contain decoration markers such as hit highlighting characters. If true,
// the strings may include markers. The default is false. To specify whether to use Unicode characters or HTML tags
// as the markers, see the
...
// Highlighting](https://docs.microsoft.com/azure/cognitive-services/bing-news-search/hit-highlighting). textFormat
// is the type of markers to use for text decorations (see the textDecorations query parameter). Possible values
// are Raw—Use Unicode characters to mark content that needs special formatting. The Unicode characters are in the
// range E000 through E019. For example, Bing uses E000 and E001 to mark the beginning and end of query terms for
...
//
// query is the user's search query string. The query string cannot be empty. The query string may contain [Bing
// Advanced Operators](http://msdn.microsoft.com/library/ff795620.aspx). For example, to limit news to a specific
// domain, use the [site:](http://msdn.microsoft.com/library/ff795613.aspx) operator. Use this parameter only with
// the News Search API. Do not specify this parameter when calling the Trending Topics API or News Category API.
// acceptLanguage is a comma-delimited list of one or more languages to use for user interface strings. The list is
// in decreasing order of preference. For additional information, including expected format, see
// [RFC2616](http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html). This header and the
// [setLang](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#setlang) query
...
// parameter and the Accept-Language query parameter only if you specify multiple languages; otherwise, you should
// use the mkt and setLang query parameters. This parameter and the mkt query parameter are mutually exclusive—do
// not specify both. count is the number of news articles to return in the response. The actual number delivered
// may be less than requested. The default is 10 and the maximum value is 100. The actual number delivered may be
// less than requested.You may use this parameter along with the offset parameter to page results. For example, if
// your user interface displays 20 articles per page, set count to 20 and offset to 0 to get the first page of
// results. For each subsequent page, increment offset by 20 (for example, 0, 20, 40). It is possible for multiple
// pages to include some overlap in results. freshness is filter news by the date and time that Bing discovered the
// news. The following are the possible filter values. Day: Return news discovered within the last 24 hours. Week:
// Return news discovered within the last 7 days. Month: Return news discovered within the last 30 days. Use this
// parameter only with the News Search API. Do not specify this parameter when calling the News Category API or the
// Trending Topics API. market is the market where the results come from. Typically, mkt is the country where the
// user is making the request from. However, it could be a different country if the user is not located in a
...
// displays 20 news per page, set count to 20 and offset to 0 to get the first page of results. For each subsequent
// page, increment offset by 20 (for example, 0, 20, 40). It is possible for multiple pages to include some overlap
// in results. originalImage is a Boolean value that determines whether the image's contentUrl contains a URL that
// points to a thumbnail of the original article's image or the image itself. If the article includes an image, and
// this parameter is set to true, the image's contentUrl property contains a URL that you may use to download the
// original image from the publisher's website. Otherwise, if this parameter is false, the image's contentUrl and
// thumbnailUrl URLs both point to the same thumbnail image. Use this parameter only with the News Search API. Do
// not specify this parameter when calling the Trending Topics API or News Category API. safeSearch is filter news
// for adult content. The following are the possible filter values. Off: Return news articles with adult text,
// images, or videos. Moderate: Return news articles with adult text but not adult images or videos. Strict: Do not
// return news articles with adult text, images, or videos. If the request comes from a market that Bing's adult
// policy requires that safeSearch is set to Strict, Bing ignores the safeSearch value and uses Strict. If you use
// the site: query operator, there is the chance that the response may contain adult content regardless of what the
// safeSearch query parameter is set to. Use site: only if you are aware of the content on the site and your
// scenario supports the possibility of adult content. setLang is the language to use for user interface strings.
// Specify the language using the ISO 639-1 2-letter language code. For example, the language code for English is
// EN. The default is EN (English). Although optional, you should always specify the language. Typically, you set
...
// header are mutually exclusive; do not specify both. A user interface string is a string that's used as a label
// in a user interface. There are few user interface strings in the JSON response objects. Also, any links to
// Bing.com properties in the response objects apply the specified language. sortBy is the order to return the news
// in. The following are the possible case-insensitive values. Date: If the request is through the News Search API,
// the response returns news articles sorted by date from the most recent to the oldest. If the request is through
// the News Trending Topics API, the response returns trending topics sorted by date from the most recent to the
// oldest. textDecorations is a Boolean value that determines whether display strings contain decoration markers
// such as hit highlighting characters. If true, the strings may include markers. The default is false. To specify
...
// your user interface displays 20 articles per page, set count to 20 and offset to 0 to get the first page of
// results. For each subsequent page, increment offset by 20 (for example, 0, 20, 40). It is possible for multiple
// pages to include some overlap in results. market is the market where the results come from. Typically, mkt is
...
// displays 20 news per page, set count to 20 and offset to 0 to get the first page of results. For each subsequent
// page, increment offset by 20 (for example, 0, 20, 40). It is possible for multiple pages to include some overlap
// in results. safeSearch is filter news for adult content. The following are the possible filter values. Off:
// Return news articles with adult text, images, or videos. Moderate: Return news articles with adult text but not
// adult images or videos. Strict: Do not return news articles with adult text, images, or videos. If the request
...
// header are mutually exclusive; do not specify both. A user interface string is a string that's used as a label
// in a user interface. There are few user interface strings in the JSON response objects. Also, any links to
// Bing.com properties in the response objects apply the specified language. since is the Unix epoch time (Unix
// timestamp) that Bing uses to select the trending topics. Bing returns trending topics that it discovered on or
// after the specified date and time, not the date the topic was published. To use this parameter, also specify the
// sortBy parameter. Use this parameter only with the News Trending Topics API. Do not specify this parameter when
// calling the News Search API or News Category API. sortBy is the order to return the news in. The following are
// the possible case-insensitive values. Date: If the request is through the News Search API, the response returns
// news articles sorted by date from the most recent to the oldest. If the request is through the News Trending
// Topics API, the response returns trending topics sorted by date from the most recent to the oldest.
// textDecorations is a Boolean value that determines whether display strings contain decoration markers such as
// hit highlighting characters. If true, the strings may include markers. The default is false. To specify whether
// to use Unicode characters or HTML tags as the markers, see the
// [textFormat](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#textformat)
// query parameter. For information about hit highlighting, see [Hit
// Highlighting](https://docs.microsoft.com/azure/cognitive-services/bing-news-search/hit-highlighting). textFormat
// is the type of markers to use for text decorations (see the textDecorations query parameter). Possible values
