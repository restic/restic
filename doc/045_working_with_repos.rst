...
between ``restic backup`` runs, you can use the option ``--stream`` to make full use of
the ``--pack-size`` option. Usually newly created packfiles are saved when the ``copy``
operation for one snapshot ends. The option ``--stream`` disregards these snapshot boundaries
