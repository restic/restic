...
The stats command returns a single JSON object for the modes ``restore-size``, ``raw-data``,
``blobs-per-file`` and ``files-by-contents``,
but the same name means different things depending on the  selected mode.
...
The ``stats --mode info`` command returns a single JSON object. Since it contains more than
40 elements, it is nested.
...
+------------------------------+-----------------------------------------------------+
| ``blobs``                    | blob counters and their sizes                       |
+------------------------------+-----------------------------------------------------+
| ``trees``                    | counts various node types, and the number of trees  |
+------------------------------+-----------------------------------------------------+
| ``packfiles``                | counts tree and data packfiles and their sizes      |
+------------------------------+-----------------------------------------------------+
| ``compression``              | overview of compression ratios and space savings    |
+------------------------------+-----------------------------------------------------+
| ``fully_unused``             | totally unreferenced packfiles and their counts     |
+------------------------------+-----------------------------------------------------+

``general`` part:

+------------------------------+-----------------------------------------------------+---------+
| ``snapshots``                | snapsshot count                                     | int     |
+------------------------------+-----------------------------------------------------+---------+
| ``size_snapshots``           | snapsshot size, if available                        | uint64  |
+------------------------------+-----------------------------------------------------+---------+
| ``tree_roots``               | number of tree roots                                | int     |
+------------------------------+-----------------------------------------------------+---------+
| ``index_files``              | number of index files, if available                 | int     |
+------------------------------+-----------------------------------------------------+---------+
| ``size_index_files``         | size of index files, if available                   | uint64  |
+------------------------------+-----------------------------------------------------+---------+

``unique_files`` part:

+------------------------------+-----------------------------------------------------+---------+
| ``unique_files_by_contents`` | unique file count                                   | int     |
+------------------------------+-----------------------------------------------------+---------+
| ``size_unique_files``        | size of unique files                                | uint64  |
+------------------------------+-----------------------------------------------------+---------+

``blobs`` part:

+--------------------------------+-----------------------------------------------------+---------+
| ``total_indexed_blobs``        | all blobs in repository                             | int     |
+--------------------------------+-----------------------------------------------------+---------+
| ``total_size``                 | size of all blobs as stored on disk, also unused ...| uint64  |
+--------------------------------+-----------------------------------------------------+---------+
| ``used_blobs``                 | used blobs in repository                            | int     |
+--------------------------------+-----------------------------------------------------+---------+
| ``used_size``                  | size of all used blobs as stored on disk            | uint64  |
+--------------------------------+-----------------------------------------------------+---------+
| ``unused_blobs``               | unused blobs in repository                          | int     |
+--------------------------------+-----------------------------------------------------+---------+
| ``unused_size``                | size of all unused blobs as stored on disk          | uint64  |
+--------------------------------+-----------------------------------------------------+---------+
| ``duplicate_blobs``            | duplicate blobs in repository                       | int     |
+--------------------------------+-----------------------------------------------------+---------+
| ``size_duplicates``            | size of all duplicate blobs as stored on disk       | uint64  |
+--------------------------------+-----------------------------------------------------+---------+
| ``tree_blobs``                 | tree blobs in repository                            | int     |
+--------------------------------+-----------------------------------------------------+---------+
| ``tree_size``                  | size of all tree blobs as stored on disk            | uint64  |
+--------------------------------+-----------------------------------------------------+---------+
|``uncompressed_size_tree_blobs``| size of all tree blobs, uncompressed                | uint64  |
+--------------------------------+-----------------------------------------------------+---------+
| ``data_blobs``                 | data blobs in repository                            | int     |
+--------------------------------+-----------------------------------------------------+---------+
| ``size_duplicates``            | size of all data blobs as stored on disk            | uint64  |
+--------------------------------+-----------------------------------------------------+---------+
...
+----------------------------------+-------------------------------------------------------+----------+
| ``total_packfiles``              | total number of packfiles                             | int      |
+----------------------------------+-------------------------------------------------------+----------+
| ``tree_packfiles``               | packfiles which contain tree data                     | int      |
+----------------------------------+-------------------------------------------------------+----------+
| ``size_tree_packfiles``          | size of tree packfiles                                | uint64   |
+----------------------------------+-------------------------------------------------------+----------+
| ``data_packfiles``               | packfiles which contain file data                     | int      |
+----------------------------------+-------------------------------------------------------+----------+
| ``size_data_packfiles``          | size of data packfiles                                | uint64   |
+----------------------------------+-------------------------------------------------------+----------+
| ``full_packfiles``               | count of packfiles which are completely used          | int      |
+----------------------------------+-------------------------------------------------------+----------+
| ``size_full_packfiles``          | size of packfiles which are completely used           | uint64   |
+----------------------------------+-------------------------------------------------------+----------+
| ``partial_packfiles``            | count of packfiles which are partly used              | int      |
+----------------------------------+-------------------------------------------------------+----------+
...
+------------------------------------+-------------------------------------------------------+----------+
| ``used_uncompressed_size``         | size of used blobs in repository in uncompressed mode | uint64   |
+------------------------------------+-------------------------------------------------------+----------+
| ``compression_ratio``              | compression ratio                                     | float64  |
+------------------------------------+-------------------------------------------------------+----------+
| ``compression_progress``           | how much of the repository is already compressed      | float64  |
+------------------------------------+-------------------------------------------------------+----------+
| ``compression_space_saving``       | how much space saved compression                      | float64  |
+------------------------------------+-------------------------------------------------------+----------+
...
+----------------------------------+-------------------------------------------------------+----------+
| ``unused_packfiles_blobs``       | count fully unreferenced blobs in these packfiles     | int      |
+----------------------------------+-------------------------------------------------------+----------+
| ``unused_packfiles_size``        | size fully unreferenced packfiles                     | uint64   |
+----------------------------------+-------------------------------------------------------+----------+
| ``unused_packfiles_blobs_size``  | size fully unreferenced blobs in these packfiles      | uint64   |
+----------------------------------+-------------------------------------------------------+----------+
