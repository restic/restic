...
func testRunDumpAssumeFailure(t testing.TB, opts DumpOptions, gopts GlobalOptions, args []string) error {
	return withTermStatus(gopts, func(ctx context.Context, term *termstatus.Terminal) error {
		return runDump(ctx, opts, gopts, args)
	})
}

// testRunDump runs the dump command and expects it to succeed
func testRunDump(t testing.TB, opts DumpOptions, gopts GlobalOptions, args []string) {
	err := testRunDumpAssumeFailure(t, opts, gopts, args)
	rtest.OK(t, err)
}

// captureStdout captures stdout while executing the provided function
...
func TestDumpErrorCases(t *testing.T) {
	...
	dumpOpts := DumpOptions{}
	err := testRunDumpAssumeFailure(t, dumpOpts, env.gopts, []string{"invalid-id", "testfile.txt"})
	rtest.Assert(t, err != nil, "Expected error for invalid snapshot ID, got nil")

	// Test case 2: Non-existent file
	err = testRunDumpAssumeFailure(t, dumpOpts, env.gopts, []string{snapshotID.String(), "non-existent-file.txt"})
	...
	err = testRunDumpAssumeFailure(t, dumpOpts, env.gopts, []string{snapshotID.String(), filepath.Join(filepath.Base(env.testdata), "testfile.txt")})
	rtest.Assert(t, err != nil, "Expected error for invalid archive format, got nil")

	// Test case 4: No arguments provided
	err = testRunDumpAssumeFailure(t, DumpOptions{}, env.gopts, []string{})
