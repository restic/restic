...
func testRunSnapshotsOpts(t testing.TB, opts SnapshotOptions, gopts global.Options) (snapmap map[restic.ID]Snapshot) {
	buf, err := withCaptureStdout(t, gopts, func(ctx context.Context, gopts global.Options) error {
		gopts.JSON = true

		// list all snapshots in JSON mode
		return runSnapshots(context.TODO(), opts, gopts, []string{}, gopts.Term)
	})
	rtest.OK(t, err)
