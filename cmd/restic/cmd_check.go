...
func runCheck(ctx context.Context, opts CheckOptions, gopts GlobalOptions, args []string, term *termstatus.Terminal) error {
	...
	errorsFound := false
	result.HintRepairIndex = false
	result.HintPrune = false
	...
	if len(errs) > 0 {
		...
		result.NumIndexErrors = len(errs)
		result.HintRepairIndex = true
	...
	result.NumTreeErrors = 0
	for err := range errChan {
		errorsFound = true
		if e, ok := err.(*checker.TreeError); ok {
			printer.E("error for tree %v:\n", e.ID.Str())
			for _, treeErr := range e.Errors {
				result.NumTreeErrors++
				printer.E("  %v\n", treeErr)
			}
		} else {
			printer.E("error: %v\n", err)
	...
	if opts.CheckUnused {
		...
		result.NumUnusedBlobs = len(unused)
	...
	if len(salvagePacks) > 0 {
		...
		for id := range salvagePacks {
			result.BrokenPacks = append(result.BrokenPacks, id.String())
		}
		printer.E("restic repair packs %v\nrestic repair snapshots --forget\n\n", strings.Join(result.BrokenPacks, " "))
	...
	if gopts.JSON {
		term.Print(ui.ToJSONString(result))
	}
	if errorsFound {
		if len(salvagePacks) == 0 {
...
type checkResult struct {
	...
	NumIndexErrors  int      `json:"num_index_errors"`
	NumPackErrors   int      `json:"num_pack_errors"`
	NumTreeErrors   int      `json:"num_tree_errors"`
	NumUnusedBlobs  int      `json:"num_unused_blobs"`
	NumDataErrors   int      `json:"num_data_errors"`
