...
func TestAddFileProgress(t *testing.T) {
	...
	result, items, _ := testProgress(func(progress *Progress) bool {
		progress.AddProgress("test.txt", fileSize, restic.NodeTypeFile)
		return false
	})
	test.Equals(t, printerTrace{
...
func TestAddDirProgress(t *testing.T) {
	result, items, _ := testProgress(func(progress *Progress) bool {
		progress.AddProgress("test-dir", 0, restic.NodeTypeDir)
		return false
	})
	test.Equals(t, printerTrace{
...
func TestMultipleItems(t *testing.T) {
	...
	result, items, _ := testProgress(func(progress *Progress) bool {
		progress.AddProgress("test-dir", 0, restic.NodeTypeDir)
		progress.AddProgress("test1.txt", fileSize, restic.NodeTypeFile)
		progress.AddProgress("test2.txt", fileSize*2, restic.NodeTypeFile)
		return false
	})
	test.Equals(t, printerTrace{
...
func TestSymlinkProgress(t *testing.T) {
	result, items, _ := testProgress(func(progress *Progress) bool {
		progress.AddProgress("test-symlink", 0, restic.NodeTypeSymlink)
		return false
	})
	test.Equals(t, printerTrace{
