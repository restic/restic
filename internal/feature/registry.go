...
func init() {
	Flag.SetFlags(map[FlagName]FlagDesc{
		...
		DeviceIDForHardlinks:    {Type: Alpha, Description: "store deviceID only for hardlinks and replace it with a virtual deviceID that stays the same across backup runs. This reduces metadata changes for example when using btrfs subvolumes or ZFS snapshots. Will be removed in a future restic version after repository format 3 is available"},
