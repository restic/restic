...
func testWrapCheckPack(ctx context.Context, t *testing.T, repo *repository.Repository,
	...
	test.OK(t, err)
	return repository.CheckPack(ctx, repo, packID, blobs, size, bufRd, dec)
}
...
func TestGapInBlobs(t *testing.T) {
	...
	test.Assert(t, len(errs) == 0, "expected no errors loading index")
...
// these test were generated by claude.ai, sonnet 4.6
...
// mustLoadSnapshots calls LoadSnapshots and fails the test on error.
// It must be called before checkData in any test that does not go through
// checkStruct first, because checker.ReadPacks calls IsFiltered() which
// dereferences the snapshotFilter pointer that LoadSnapshots initialises.
func mustLoadSnapshots(t *testing.T, chkr *checker.Checker) {
	t.Helper()
	err := chkr.LoadSnapshots(context.TODO(), &data.SnapshotFilter{}, nil)
	test.OK(t, err)
}
...
func setupCheckRepo(t *testing.T, wrap func(backend.Backend) backend.Backend) *checker.Checker {
	t.Helper()
	...
	checkRepo := repository.TestOpenBackend(t, wrap(be))
	chkr := checker.New(checkRepo, false)
...
func TestCheckPackHashMismatch(t *testing.T) {
	...
	test.Assert(t, found, "expected 'unexpected pack id' error, got: %v", dataErrs)
}
...
func TestCheckPackDownloadError(t *testing.T) {
	...
	for _, err := range dataErrs {
		var packErr *repository.ErrPackData
		test.Assert(t, !errors.As(err, &packErr),
			"complete download failure must NOT produce ErrPackData, got: %v", err)
		test.Assert(t, strings.Contains(err.Error(), "download error"),
