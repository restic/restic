...
// hardlinkedOn returns overrides that make the named files look like they are
// hardlinked and located on the device deviceID.
func hardlinkedOn(deviceID uint64, names ...string) map[string]deviceOverride {
	overrides := make(map[string]deviceOverride)
	for _, name := range names {
		overrides[name] = deviceOverride{deviceID: deviceID, links: 2}
	}
	return overrides
}

// snapshotWithDeviceOverrides backs up target like snapshot does, but with
// the device ID and link count of the files in overrides replaced.
func snapshotWithDeviceOverrides(t testing.TB, repo archiverRepo, parent *data.Snapshot, target string, overrides map[string]deviceOverride) (*data.Snapshot, *data.Node) {
	t.Helper()

	filesystem := &deviceOverrideFS{FS: fs.NewLocal(), overrides: overrides}
	return snapshot(t, repo, filesystem, parent, target)
}
...
func TestVirtualDeviceID(t *testing.T) {
	...
	sn, node := snapshotWithDeviceOverrides(t, repo, nil, "testfile", hardlinkedOn(0x1000, "testfile"))
	...
	sn, nodeAgain := snapshotWithDeviceOverrides(t, repo, sn, "testfile", hardlinkedOn(0x1000, "testfile"))
	...
	_, nodeRemounted := snapshotWithDeviceOverrides(t, repo, sn, "testfile", hardlinkedOn(0x2000, "testfile"))
	...
	_, nodeNoParent := snapshotWithDeviceOverrides(t, repo, nil, "testfile", hardlinkedOn(0x3000, "testfile"))
...
func TestVirtualDeviceIDUnchangedTree(t *testing.T) {
	...
	sn, _ := snapshotWithDeviceOverrides(t, repo, nil, "testdir", hardlinkedOn(0x1000, "testdir", "testfile"))
	// The device ID changed. The tree must stay identical to avoid uploading
	// new tree blobs.
	snRemounted, _ := snapshotWithDeviceOverrides(t, repo, sn, "testdir", hardlinkedOn(0x2000, "testdir", "testfile"))
...
func TestVirtualDeviceIDNotChangedByFileWithoutHardlinks(t *testing.T) {
	...
	backup := func(parent *data.Snapshot, deviceID uint64) map[string]*data.Node {
		t.Helper()

		overrides := hardlinkedOn(deviceID, "testdir", "testfile")
		overrides["other"] = deviceOverride{deviceID: deviceID, links: 1}
		_, dir := snapshotWithDeviceOverrides(t, repo, parent, "testdir", overrides)
		return loadTreeNodes(t, repo, *dir.Subtree)
	}

	sn, _ := snapshotWithDeviceOverrides(t, repo, nil, "testdir", hardlinkedOn(0x1000, "testdir", "testfile"))
	nodes := backup(nil, 0x1000)
...
func TestVirtualDeviceIDUpgrade(t *testing.T) {
	...
	sn, node := func() (*data.Snapshot, *data.Node) {
		...
		return snapshotWithDeviceOverrides(t, repo, nil, "testfile", hardlinkedOn(0x1000, "testfile"))
	}()
	...
	_, upgraded := snapshotWithDeviceOverrides(t, repo, sn, "testfile", hardlinkedOn(0x2000, "testfile"))
...
func TestVirtualDeviceIDMultipleDevices(t *testing.T) {
	...
	files := TestDir{
		"testdir": TestDir{
			"first":  TestDir{"firstfile": TestFile{Content: "first device"}},
			"second": TestDir{"secondfile": TestFile{Content: "second device"}},
		},
	}
	tempdir, repo := prepareTempdirRepoSrc(t, files)
	...
	backup := func(parent *data.Snapshot, firstDev, secondDev uint64) (*data.Snapshot, map[string]*data.Node) {
		...
		overrides := hardlinkedOn(firstDev, "firstfile")
		maps.Copy(overrides, hardlinkedOn(secondDev, "secondfile"))
		sn, dir := snapshotWithDeviceOverrides(t, repo, parent, "testdir", overrides)
