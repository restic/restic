...
func (plan *PrunePlan) Execute(ctx context.Context, printer progress.Printer) error {
	...
	if len(plan.removePacksFirst) != 0 {
		...
		_ = deleteFiles(ctx, true, &unpackedRepository{repo}, plan.removePacksFirst, restic.PackFile, printer)
		// forget unused data
	...
	if plan.opts.UnsafeRecovery {
		...
		err := deleteFiles(ctx, false, &unpackedRepository{repo}, indexFiles, restic.IndexFile, printer)
		if err != nil {
	...
	if len(plan.removePacks) != 0 {
		...
		_ = deleteFiles(ctx, true, &unpackedRepository{repo}, plan.removePacks, restic.PackFile, printer)
	}
	if ctx.Err() != nil {
		return ctx.Err()
	}
	...
	if plan.opts.UnsafeRecovery {
		err := repo.idx.SaveFallback(ctx, &unpackedRepository{repo}, plan.ignorePacks, printer.NewCounter("packs processed"))
		if err != nil {
			return errors.Fatalf("%s", err)
		}
	}
