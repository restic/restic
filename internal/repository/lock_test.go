...
func createFakeLock(repo *Repository, t time.Time, pid int) (restic.ID, error) {
	...
	return restic.SaveJSONUnpacked(context.TODO(), &unpackedRepository{repo}, restic.LockFile, &newLock)
}
...
func removeLock(repo *Repository, id restic.ID) error {
	return (&unpackedRepository{repo}).RemoveUnpacked(context.TODO(), restic.LockFile, id)
}
