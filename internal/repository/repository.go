...
// unpackedRepository allows using SaveUnpacked and RemoveUnpacked with all FileTypes
type unpackedRepository struct {
	*Repository
}
...
func (r *unpackedRepository) SaveUnpacked(ctx context.Context, t restic.FileType, buf []byte) (id restic.ID, err error) {
...
func (r *unpackedRepository) RemoveUnpacked(ctx context.Context, t restic.FileType, id restic.ID) error {
...
func (r *Repository) Flush(ctx context.Context) error {
	...
	return r.idx.SaveIndex(ctx, &unpackedRepository{r})
}
...
func (r *Repository) init(ctx context.Context, password string, cfg restic.Config) error {
	...
	r.keyID = key.ID()
	r.setConfig(cfg)
	return restic.SaveConfig(ctx, &unpackedRepository{r}, cfg)
}
