...
func OpenRepository(ctx context.Context, gopts Options, printer progress.Printer) (*repository.Repository, error) {
	...
	err = authenticateWithRepository(ctx, s, &gopts, printer)
	if err != nil {
		return nil, err
	}
...
// authenticateWithRepository handles password reading and decrypts the repository.
func authenticateWithRepository(ctx context.Context, s *repository.Repository, gopts *Options, printer progress.Printer) error {
	passwordTriesLeft := 1
